...
#### H1. SceneEditor::OnEnter 未重置 m_shiftHeld
...
#### M1. ResourceManager Unload 函数的反向查找可优化
...
#### M2. Config::Save() 在每次 Set 时频繁刷盘
- **文件**: `src/core/config.cpp`
- **问题**: 每次修改配置后立即写入磁盘，主题切换等批量操作会多次 I/O
- **优化建议**: 延迟保存（dirty flag + 定时/退出时刷盘）或批量保存接口

#### M3. Input 模块硬编码默认分辨率 1920×1080
...
#### M4. 粒子系统 DrawSakuraBlossom 每帧重新计算常量
...
#### L2. editor_command.h MAX_HISTORY 硬编码
- 撤销历史上限 200 应改为可配置项

#### L3. database.h 硬编码数据库路径
- `"data/sakura.db"` 应从 Config 读取或支持环境变量

#### L4. Screen_shake.cpp 每次 Trigger 创建 random_device
- 应使用持久化的随机引擎
...
- ⚠️ 渲染器文字性能需要缓存层（见 C2）
- ⚠️ ResourceManager 的四种资源各用独立 map，结构可统一
...
- ✅ 包含编辑器、校准、教程等辅助场景
- ⚠️ 重玩按钮丢失难度选择（见 C1）
- ⚠️ 编辑器 m_shiftHeld 重入时未重置（见 H1）

### Audio 模块 (src/audio/)
- ✅ 使用 miniaudio 作为底层引擎
- ✅ 音频可视化支持 Bar/Circle/Wave 三种模式
- ✅ SFX 生成器可程序化合成打击音效
- ⚠️ Handle 播放 API 未实现（见 H2）
- ⚠️ 频谱分析性能（见 M5）
...
| 项目 | 优先级 | 说明 |
|------|--------|------|
| 修复重玩按钮丢失难度 (C1) | 🔴 | 功能 Bug |
| 实现渲染器文字缓存 (C2) | 🔴 | 性能瓶颈 |
| 修复测试框架警告 (C3) | 🔴 | 编译警告 |
| 赋值 playTimeSeconds (H3) | 🟠 | 数据缺失 |
| 统一版本号 (H5) | 🟠 | 信息不一致 |
| 编辑器 m_shiftHeld 重置 (H1) | 🟠 | 状态残留 |
| Handle 播放 API 实现 (H2) | 🟠 | API 缺失 |
...
1. ✅ 先修复 Critical 和 High 问题（本 PR 处理部分）
