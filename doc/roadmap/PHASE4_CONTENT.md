...
- Step 4.1 新手教程已接入首启引导、4 课流程、失败重试与完成回主菜单流程。
- Step 4.2 官方测试谱面已补齐 `tutorial_song` / `spring_breeze` / `cherry_blossom` / `digital_dream` / `sakura_storm`，均使用 v2 格式、静音 WAV 与占位封面。
- Step 4.3~4.6 成就、PP、统计场景与音频可视化已实现并纳入当前 v0.5.3-alpha 内容基线。
- 验收测试已覆盖 Phase 4 官方谱面的扫描、加载、难度、音符数量、Hold/Slider、SV 与 BPM 变化要求。
