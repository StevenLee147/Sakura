...
TEST_CASE("教程共四课且顺序正确", "[tutorial][data]")
{
    auto lessons = BuildTutorialLessons();
    REQUIRE(lessons.size() == 4);
    ...
    REQUIRE(lessons[3].type == TutorialLessonType::Mixed);
}
...
TEST_CASE("教程脚本满足需求中的音符数量与容差", "[tutorial][spec]")
{
    ...
    REQUIRE(lessons[3].notes.size() == 6);
    REQUIRE(lessons[3].usesKeyboard == true);
