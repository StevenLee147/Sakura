...
### 0.按流程工作
- **先查看现有代码**：在实现新功能或修改功能前，先查看工作区内是否已有相关实现可以复用或参考
- **先查看报错信息**：当出现错误时，先仔细查看编译器或运行时的报错信息，定位问题所在，再进行修改
- **先查看文档**：在使用新库或API时，先查看官方文档或已有的使用示例，确保正确使用
- **先查看指令**：在进行任何修改前，先查看本指令文件，确保遵循项目的代码规范和开发流程
- **按步骤进行**：按照指令中的步骤和要求进行开发，确保代码质量和项目一致性
- **保证测试通过**：在提交代码前，确保所有相关的测试都能通过，避免引入新的错误
- **提交git**：在完成修改后，使用git提交代码，并在提交信息中简要说明修改内容和原因
- **版本号**：在每次提交后，根据修改的内容和范围，更新项目的版本号（如有需要），git tag版本号和游戏内版本号要同步更新

- **总体流程**：
  1. 查看现有代码和指令，了解当前实现和规范
  2. 根据需求分析和设计，确定需要修改或新增的功能
  3. 编写代码实现功能，遵循命名规范和代码组织原则
  4. 编译并运行代码，查看报错信息并进行调试修改
  5. 确保所有相关测试通过，验证功能正确性
  6. 提交代码到git，并更新git tag版本号和游戏内版本号（如有需要）
